...
save_dir='/nfs/nhome/live/glindsay/larger_damCV/nets/'
loss_dir='/nfs/nhome/live/glindsay/larger_damCV/nets/losses/'


def build_resnet(num_channels=[16,32,64],num_blocks=[2,2,2],l2_reg=None,reg=None,inits='glorot_uniform',last_layer=True, input_shape=(266,266,4),fl_size=7,dense=128):
...
def binary_classifier_net(model_num,batch_size=256,save=True,epochs=10,l2_reg=.001,inits='truncated_normal',reg=None,LR=.001,opt='adam',dense=128,noise_std=0):

    output_dim = 1
    net_name = 'SupDamBin_mn'+model_num
    act_f = 'sigmoid' 
    ...
    try:
        ...
        net_name += '_L2'+str(l2_reg)+'_IN'+str(inits)+'_ar'+str(reg)+'_lr'+str(LR)+'_OP'+str(opt) #+'_al'+str(alpha)+'_gm'+str(gamma)+'_'
        reloaded = False
        inputs, outputs = build_resnet(l2_reg=l2_reg,inits=inits,reg=reg,num_channels=[32,32,16],num_blocks=[2,2,2],fl_size=3) 
        ...
        outputs = add_FC_layer(outputs,output_dim,activation=act_f,use_bias=True,inits=inits) #^last changed to32for10
    ...
    learning_rate_fn = tf.keras.optimizers.schedules.PiecewiseConstantDecay([15*(DataGen.batch_per_epoch),30*(DataGen.batch_per_epoch)], [LR,LR/10,LR/100])
    ...
    #train model
    model.fit(DataGen,verbose=2,epochs=epochs,steps_per_epoch=DataGen.batch_per_epoch,validation_data=(val_im,val_lab), validation_steps=None,validation_freq=1, callbacks=[csv_logger])
